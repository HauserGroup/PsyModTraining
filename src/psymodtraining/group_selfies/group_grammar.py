...
from psymodtraining.group_selfies.constants import INDEX_CODE, POP_SYMBOL
from psymodtraining.group_selfies.group_mol_graph import Group, GroupWrapper
from psymodtraining.group_selfies.utils.group_utils import extract_group_bonds
...
import psymodtraining.group_selfies as group_selfies
...
def name_to_token(name, n_points):
    return [
        f'[:{"".join(map(str, bond_type))}{name}]'
        for bond_type in itertools.product(["", "=", "#"], range(n_points))
    ]
...
class GroupGrammar:
            ...
            [
                (
                    ...
                    Group(
                        n,
                        s,
                        all_attachment=all_at,
                        priority=priority[0] if len(priority) else 0,
                    ),
                )
                for n, s, all_at, *priority in l
            ]
        )
    ...
    def from_file(cls, filename):
        ...
        vocab_dict = dict(
            [
                (n, Group(n, s, priority=int(priority[0]) if len(priority) else 0))
                for n, s, *priority in test_gs
            ]
        )
        return cls(vocab=vocab_dict)
    ...
    def to_file(self, filename):
        open(filename, "w").write(
            "\n".join(
                [
                    " ".join([n, group.canonsmiles, str(group.priority)])
                    for n, group in self.vocab.items()
                ]
            )
        )
    ...
    def names_sorted_by_size(self):
        ...
        return sorted(
            names,
            reverse=True,
            key=lambda x: (self.vocab[x].priority, self.vocab[x].group_size),
        )
    ...
    def all_tokens(self):
        return list(
            chain.from_iterable(
                [
                    name_to_token(name, len(group.attachment_points))
                    for name, group in self.vocab.items()
                ]
            )
        )
    ...
    def encoder(self, mol, groups, **args):
        from psymodtraining.group_selfies.group_encoder import group_encoder

        return group_encoder(self, mol, groups, **args)

    def decoder(self, selfies, **args):
        from psymodtraining.group_selfies.group_decoder import group_decoder
    ...
    def pretty_encode(self, gselfi, **args):
        return self.full_encoder(
            Chem.MolFromSmiles(Chem.MolToSmiles(self.decoder(gselfi))), **args
        )
