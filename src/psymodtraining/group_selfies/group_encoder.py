...
from psymodtraining.group_selfies.constants import INDEX_ALPHABET, ORGANIC_SUBSET
from psymodtraining.group_selfies.grammar_rules import get_selfies_from_index
from psymodtraining.group_selfies.group_mol_graph import MolecularGraph
...
def derive_ring_bond(G, bond, map_idx):
    ...
    src_idx = (
        map_idx[bond.src]
        if bond.src in map_idx
        else (
            map_idx[f"group{(g_idx := G[bond.src].group_tag[0])}"]
            + bond.group_idx_dict[g_idx]
        )
    )
    dst_idx = (
        map_idx[bond.dst]
        if bond.dst in map_idx
        else (
            map_idx[f"group{(g_idx := G[bond.dst].group_tag[0])}"]
            + bond.group_idx_dict[g_idx]
        )
    )
...
def graph_to_selfies(
    grammar, G, previous_bond, root, counter=None, map_idx=None
):  # assume extracted groups are already pointed
    derived = []
    curr = root
    ...
    map_idx = map_idx or {}
    counter = (
        counter or MutableCounter(0)
    )  # use mutable class instead of int so that counter increments in recursion without additional returns
    while True:
        if G[curr].group_tag is None:
            ...
            for i, bond in enumerate(out_bonds):
                if bond.dst in G.visited:
                    ...
                    branch = graph_to_selfies(
                        grammar, G, bond, bond.dst, counter=counter, map_idx=map_idx
                    )
            ...
            for bond in branches:
                this_inner_idx = bond.group_idx_dict[
                    group_obj.index
                ]  # index relative to attachment points array
                rel_idx = (this_inner_idx - prev_inner_idx) % len(group_obj)
                prev_inner_idx = this_inner_idx

                derived.extend(
                    [get_selfies_from_index(len(INDEX_ALPHABET) - 1)[0], "[pop]"]
                    * (rel_idx // (len(INDEX_ALPHABET) - 1))
                )
                derived.append(
                    get_selfies_from_index(rel_idx % (len(INDEX_ALPHABET) - 1))[0]
                )  # parse into chunks
...
def atom_to_selfies(bond, atom):
    ...
    elif atom.h_count == 0 and not (
        atom.element in ORGANIC_SUBSET and atom.charge == 0 and atom.radical == 0
    ):
        builder.append("H0")
