...
from psymodtraining.group_selfies.bond_constraints import get_bonding_capacity
from psymodtraining.group_selfies.utils.group_utils import (
    HashableMolecule,
    bond_to_order,
    get_core,
    hash_molecule,
...
def fragment_mol(
    ...
    if alias_translation[method.lower()] == "default":
        ...
        bs = [
            m.GetBondBetweenAtoms(a1, a2).GetIdx()
            for a1, a2 in m_clone.GetSubstructMatches(fragment_smarts)
        ]
        if not bs:
            return []
        fragments = Chem.GetMolFrags(
            Chem.FragmentOnBonds(m_clone, bs), asMols=True, sanitizeFrags=True
        )
        ...
        fragments = [
            res
            for mult in FraggleSim.generate_fraggle_fragmentation(m)
            for x in mult.split(".")
            if (res := Chem.MolFromSmiles(x)) is not None
        ]
    else:
        raise ValueError("fragments is unbound")
    ...
    for fragment in fragments:
        ...
        for idx, atom in enumerate(new_f.GetAtoms()):
            if not atom.GetAtomicNum():
                ...
                valence = bond_to_order[
                    fragment.GetBondBetweenAtoms(parent, idx).GetBondType()
                ]
...
def get_attachment_points(atom):
    return [
        neigh.GetIntProp("valAvailable")
        for neigh in atom.GetNeighbors()
        if neigh.HasProp("valAvailable")
    ]
...
def merge_list(L1, L2, limit=math.inf):
    ...
    L1.sort(
        reverse=True
    )  # sort lists descending, assume that optimal merging is "greedy"
...
def capacity(atom):
    ...
    for neigh in atom.GetNeighbors():
        if neigh.GetSymbol() == "*":
            explicit_valence -= (
                atom.GetOwningMol()
                .GetBondBetweenAtoms(atom.GetIdx(), neigh.GetIdx())
...
def merge_patterns(p1, p2, core):  # return None if not mergeable, else return merged
    canon_mapping = p1.GetSubstructMatch(
        core
    )  # assign an arbitrary mapping for pattern 1
    best_mappings = []
    for match_mapping in p2.GetSubstructMatches(core, uniquify=False):
        ...
        for core_idx, (c_idx, m_idx) in enumerate(zip(canon_mapping, match_mapping)):
            ...
            if (
                res := merge_list(
                    c_attach, m_attach, limit=min(capacity(a1), capacity(a2))
                )
            ) is not None:
    ...
    if not best_mappings:  # if no valid mapping, then not mergeable
        return None
    return sorted(best_mappings, reverse=True, key=lambda x: x[0])[0][
        1
    ]  # use mapping with most intersections
...
def closely_contained(mol, substruct, k=2):
    ...
    for x in range(mol.GetNumAtoms()):
        ...
        if len(mol.GetAtomWithIdx(x).GetNeighbors()) != 1:
            ...
            mol.GetAtomWithIdx(x).GetBonds()[0].GetBondType() != AllChem.BondType.SINGLE
        ):
            return False
    return True
...
def force_core(mol, core):
    ...
    for idx, atom in enumerate(new_m.GetAtoms()):
        if idx not in mapping:
            if any(
                in_mapping := [
                    neigh.GetIdx() in mapping for neigh in atom.GetNeighbors()
                ]
            ):
...
def fragment_mols(
    m_set, convert=False, n_limit=10, target=100, fragmented=False, method="default"
):
    ...
    for m in tqdm(m_set):
        ...
        for new_f, new_f_core in (
            fragment_mol(m, method=method) if not fragmented else [(m, get_core(m))]
        ):
        ...
        for existing_core in current_cores:
            if closely_contained(core.mol, existing_core.mol):
                for group in core_dict[core]:
                    merge_into_core(
                        existing_core,
                        force_core(group, existing_core.mol),
                        existing_core.mol,
                    )
                break
        else:
            ...
            chosen := temp_set.pop(
                max(
                    ...
                    key=lambda x: sum(
                        [
                            dist_matrix[
                                pair_to_triangular_idx(idx, temp_set[x][1], len(mf))
                            ]
                            * weights[idx]
                            * weights[temp_set[x][1]]
                            for _, idx in result
                        ]
                    ),
                )
