...
def verify_group(pat, group=False):
    ...
    for atom in pat.GetAtoms():  # essentially the same checks found in group_parser
        if atom.GetAtomMapNum():
            ...
            if group:
                potential_val[parent.GetIdx()] += (
                    0 if valency == 1 else valency
                )  # "unsafe" temporary patch; only works because of RDKit behaviour;
                ...
                potential_val[parent.GetIdx()] += (
                    valency - 1
                )  # attachment points always single bonded
    ...
    for k, v in potential_val.items():
        ...
        assert (
            get_bonding_capacity(a.GetSymbol(), a.GetFormalCharge())
            >= sum(
                [
                    bond.GetValenceContrib(a)
                    for bond in a.GetBonds()
                    if (not group)
                    or (
                        bond.GetBeginAtom().GetAtomicNum()
                        and bond.GetEndAtom().GetAtomicNum()
                    )
                ]
            )
            + v
...
def group_parser(s, sanitize=False):
    ...
    try:
        ...
        for i in range(1, len(mapped), 2):
            bond_symbol = (
                bond_symbs[int(mapped[i]) - 1]
                if mapped[i - 1][-1:] not in bond_symbs[1:]
                else ""
            )
            if i != 1 or mapped[0]:
                mapped[i] = (
                    f"{bond_symbol}[{(attachment_idx)}*:{mapped[i]}]"  # wildcard atoms can have custom isotopes; give each attachment point a custom isotope so that they are unique and chirality is maintained
                )
            else:
                mapped[i] = (
                    f"[{(attachment_idx)}*:{mapped[i]}]{bond_symbol}"  # wildcard atoms can have custom isotopes; give each attachment point a custom isotope so that they are unique and chirality is maintained
                )
            attachment_idx += 1
...
def map_to_smiles(s, clear_valence=False):
    ...
    return re.sub(
        r"[\-\=\#\@]?\[\d*\*:(\d)\]", r"*\1" if not clear_valence else r"*1", s
    )
...
def mol_to_group_s(m, smiles_override=None):
    ...
    if smiles_override is None:
        ...
        for atom in m_clone.GetAtoms():  # type: ignore
...
def to_smarts(s):  # assume s is verified already
    ...
    for aromatic_atom in ["C", "N", "O", "S", "P"]:
        new_s = re.sub(
            f'[{aromatic_atom.upper()}{aromatic_atom.lower()}](?=[^{"".join([x[1] for x in elem_symbs if len(x) >= 2 and x[0] == aromatic_atom])}]|$)',
            f"[#{ptable.GetAtomicNumber(aromatic_atom)}]",
            new_s,
        )
...
def extract_group_bonds(
    group, mol, matched
):  # extracts atoms and bonds for all instances of groups
    ...
    for match in mol_h.GetSubstructMatches(
        group.pattern, uniquify=False, useChirality=True
    ):
        ...
        for pattern_idx, idx in enumerate(match):
            ...
            if pattern_idx in group.parent_map:  # current atom is attachment point
                ...
                for neigh in a.GetNeighbors():
                    ...
                    if valid:
                        net_bonds[-1].append(
                            ((idx, expected_parent), pattern_idx)
                        )  # bond mapping info: ((bond_idx1 <- outer, bond_idx2 <- outer), group_pattern_idx <- inner)
                        continue
                del net_atoms[-1]
                del net_bonds[-1]
                break
                ...
                if (
                    (pat_at.GetFormalCharge() != mol_at.GetFormalCharge())
                    or (pat_at.GetNumExplicitHs() != mol_at.GetNumExplicitHs())
                    or (pat_at.GetIsotope() != mol_at.GetIsotope())
                    or (
                        pat_at.GetNumRadicalElectrons()
                        != mol_at.GetNumRadicalElectrons()
                    )
                ):
                    del net_atoms[-1]
                    del net_bonds[-1]
                ...
                if (
                    pattern_idx not in group.parent_map.values()
                ):  # if atom is not an attachment point parent, then only non attachment points are valid
                    valid_neighbours = valid_neighbours - set(
                        [match[x] for x in group.parent_map.keys()]
                    )
                for neigh in a.GetNeighbors():
                    if neigh.GetAtomicNum() == 1 and not neigh.GetIsotope():
                        continue
                    if (
                        neigh.GetIdx() not in valid_neighbours
                    ):  # check if atom has invalid neighbour
                        del net_atoms[-1]
                        del net_bonds[-1]
                        break
                else:
...
def get_core(m, keep_dict=False):  # remove all attachment points to get "core" of group
    ...
    if keep_dict:
        neighbor_dict = defaultdict(
            list, [(v[0].GetIdx(), v[1:]) for v in neighbor_dict.values()]
        )
...
def topology_from_rdkit(
    mol, only_atomic_num=False
):  # converts rdkit mol to nx graph for use in hashing. includes valAvailable prop in the atom properties.
    topology = nx.Graph()
    for atom in mol.GetAtoms():
        anstr = f"{atom.GetAtomicNum()}"
        if not only_atomic_num:
            anstr += f"{atom.GetFormalCharge()}{atom.GetNumExplicitHs()}" + (
                f"{atom.GetProp('valAvailable')}"
                if atom.HasProp("valAvailable")
                else ""
            )
...
def hash_molecule(mol1):
    return nx.weisfeiler_lehman_graph_hash(
        topology_from_rdkit(mol1, only_atomic_num=True),
        edge_attr="order",
        node_attr="an",
    )
