...
from psymodtraining.group_selfies.constants import BOND_DIR_REV
...
class DirectedBond:
    def __init__(self, src: int, dst: int, order: Union[int, float], direc: str):
        self.src = src
        self.dst = dst
        self.order = order
        self.direc = direc
...
class MolecularGraph:
    def __init__(self, mol=None, groups=[], smarts=False):
        ...
        if mol is not None:
            ...
            bond_to_order = {
                ...
                BT.TRIPLE: 3,
                BT.QUADRUPLE: 4,
            }  # add zero order bond so attachment doesnt count to valency
            for bond in mol.GetBonds():
                src, dst = bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()
                self.add_bond(
                    src,
                    dst,
                    bond_to_order[bond.GetBondType()],
                    BOND_DIR_REV[bond.GetBondDir()],
                    update_bond_counts=(
    ...
    def update_bond_order(self, bond, new_order: Union[int, float]) -> None:
    ...
    def add_group(self, group, member_idxs, bond_idxs):
        ...
        for outer, inner in member_idxs:
            assert (
                self[outer].group_tag is None
            ), f"overlapping groups: atom {outer} is already in a group"
        ...
        for (outer_dst, outer_src), attach_idx in bond_idxs:
            for i in [1, -1]:
                if tuple([outer_src, outer_dst][::i]) in self.diG.edges:
                    ...
                    bond_obj.group_idx_dict[group_idx] = group.attachment_points.index(
                        attach_idx
                    )
    ...
    def find_next_available(self, group, current_idx, rel_idx):
        ...
        circular_range = list(range(possible_idx, len(group))) + list(
            range(possible_idx)
        )
...
class Group:
    def __init__(
        ...
        canonsmiles,
        overload_index=None,
        all_attachment=False,
        smarts_override=None,
        priority=0,
        sanitize=False,
    ):
        assert "-" not in name, "Group names cannot contain dashes"
        assert (
            re.match(r"[0-9]+", name) is None
        ...
        if smarts_override:
            ...
            if all_attachment:
                ...
                parent_map = defaultdict(
                    int, [(non_attachment[k], v) for k, v in parent_map.items()]
                )
                ...
                for idx in range(counter):
                    ...
                    if temp_graph[idx].available_bonds():
                        z = min(
                            (temp_graph[idx].available_bonds() - parent_map[idx]),
                            len(order_to_bond) - 1,
                        )
                        if z > 0:
                            new_at = AllChem.Atom("*")
                ...
                canon_smi = Chem.MolToSmiles(
                    self.mol, canonical=True, kekuleSmiles=True, allBondsExplicit=True
                )
                try:
        ...
        self.attachment_points = [
            i
            for i, a in enumerate(self.pattern.GetAtoms())
            if a.HasProp("valAvailable")
        ]
        self.parent_map = {}
    ...
    def find_next_available(self, current_idx, rel_idx):
        ...
        for shift in range(len(self.attachment_points)):
            ...
            if self.mol.GetAtomWithIdx(self.attachment_points[inner]).HasProp(
                "valAvailable"
            ):
                return inner
        return None
